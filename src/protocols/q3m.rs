use errors::Error;
use models::{FollowUpQuery, FollowUpQueryProtocol, Packet, ParseResult, Protocol,
             ProtocolResultStream, TProtocol};
...
impl Protocol for ProtocolImpl {
    ...
    fn make_request(&self, _: Option<Value>) -> Vec<u8> {
        let mut out = Vec::new();
        q3a::Packet::GetServers(q3a::MasterQueryOptions {
    ...
    fn parse_response(&self, p: Packet) -> ProtocolResultStream {
        Box::new(futures::stream::iter_result(match q3a::Packet::from_bytes(
            p.data.as_slice().into(),
        ).map_err(|e| format_err!("{}", e))
            .and_then(|(_, pkt)| match pkt {
                q3a::Packet::GetServersResponse(server_list) => {
                    Ok(match self.q3s_protocol.clone() {
                        Some(q3s_protocol) => server_list
                            .into_iter()
                            .map(|addr| {
